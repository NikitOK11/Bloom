...
- `Olympiad` — deprecated legacy-слой совместимости поверх `Event`; новые product-facing данные создавать через `Event`/Событие, а не через `Olympiad`.
- `Event`/Событие — основная модель чтения для пользовательских страниц и редактирования в админке; `Olympiad` не должен быть основным UI entry point.
- Новое создание команд должно идти от `Event`; страницы событий — основной экран просмотра команд.
- `Team.event` — каноническая связь команды с `Event`/Событием; `Team.olympiad` nullable и остается только для legacy-совместимости.
- Новый product-facing код не должен напрямую зависеть от `Team.olympiad`.
